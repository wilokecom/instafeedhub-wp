...
import { INSTA_IFRAME_URL } from '../modules/contants';
...
export default function Edit( { setAttributes, isSelected, attributes, clientId } ) {
	...
	const [ postId, setPostId ] = useState( instaId );
	...
	function handleReceiveFromIframe( event ) {
		...
		if ( type === 'UPDATE-ITEM' ) {
			...
			setAttributes( {
				instaId: payload.id,
				instaTitle: payload.title,

			} );
		}
		if ( type === 'CREATE-ITEM' && event.data.payload.status.includes( 'success' ) ) {
			setPostId( event.data.payload.id );
			setAttributes( {
	...
	const renderLoading = () => {
		...
		return (
			<p className="wilBlogLoading">
				<svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true"
					focusable="false">
					<path
						fillRule="evenodd"
	...
	const renderIconSvg = () => {
		return (
			<svg enableBackground="new 0 0 24 24" height="30" viewBox="0 0 24 24" width="30"
	...
	return (
		<div className="wilIntaFeedCp">
			...
			<Button isPrimary onClick={ handleClickBtnConnect } className="wilIntaFeedCp__btnEdit"
				...
				{ verifyError }
				{ ! postId ? 'Connect and Create new a InstaFeed' : `${ instaTitle } (${ postId })` }
			</Button>
