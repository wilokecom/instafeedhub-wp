...
class AdminController
{
	...
	public function saveInstaIds($postId, $post)
	{
		...
		if (preg_match_all('/"instaId":([\d]+)/', $post->post_content, $aMatches)) {
			if (isset($aMatches[1])) {
				$aInstaFeedhubID = $aMatches[1];
			}
		}

		//VC
		if (preg_match_all('/vc-instagram-feedhub vc-instagram-feedhub-input=\"([\d]+)\"/', $post->post_content, $aMatches)) {
			if (isset($aMatches[1])) {
				$aInstaFeedhubID = array_merge($aInstaFeedhubID, $aMatches[1]);
			}
		}

		update_post_meta($postId, 'instafeedhub_ids', $aInstaFeedhubID);
	}
}
