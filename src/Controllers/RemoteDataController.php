	...
	public function updateData( \WP_REST_Request $oRequest ) {
		...
		if ( empty( $aData ) ) {
			...
			if ( isset( $aData[ $postID ] ) ) {
				if ( isset( $aParams['status'] ) && $aParams['status'] !== 'publish' ) {
					unset( $aData[ $postID ] );
				} else {
		...
		return Message::success( [
			'msg' => __( 'This post has been update successfully' )
		] );
	}
...
//				'postId'   => $postID,
//				'options'  => $aData,
//				'isExists' => is_array( $aData ) && isset( $aData[ $postID ] )
//			] );
//		}
